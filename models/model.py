...
try:
    ...
    HAS_TFC = False
    print("Warning: tensorflow_compression not found. Using standard Conv2D layers.")
from models.channellayer import RayleighChannel, AWGNChannel, RicianChannel, SatelliteChannel, GEOSatelliteChannel, LEOSatelliteChannel
...
class SemViT(tf.keras.Model):
    def __init__(self, block_types, filters, num_blocks, has_gdn=True,
        ...
        '''
        super().__init__()
        if has_gdn and HAS_TFC:
            gdn_func=tfc.layers.GDN()
            igdn_func=tfc.layers.GDN(inverse=True)
        else:
            if has_gdn and not HAS_TFC:
                print("Warning: GDN requested but TFC not available. Using Identity.")
        ...
        if channel == 'Rayleigh':
            ...
            self.channel = SatelliteChannel(snrdB=snrdB)
        elif channel == 'GEO':
            self.channel = GEOSatelliteChannel(snrdB=snrdB)
        elif channel == 'LEO':
            self.channel = LEOSatelliteChannel(snrdB=snrdB)
        else:
...
def build_conv(repetition, filter_size, kernel_size=5, stride=1, gdn_func=None):
    ...
    for i in range(repetition):
        ...
        if HAS_TFC:
            ...
            # Fallback for when TFC is missing
            x.add(tf.keras.layers.Conv2D(
                filters=filter_size,
                kernel_size=kernel_size,
                strides=s,
                padding="same",
